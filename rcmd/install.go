...
func InstallThroughBinary(
	...
	if exists {
		Log.Log.WithField("package", ir.Package).Info("package already installed")
		return CmdResult{
			ExitCode: -999,
...
func InstallPackagePlan(
	...
	iq := NewInstallQueue(ncpu,
		...
		func(iu InstallUpdate) {
			if iu.Err != nil {
				Log.Log.WithField("err", iu.Err).Warn("error installing")
				anyFailed = true
				...
				Log.Log.WithFields(logrus.Fields{"binary": iu.BinaryPath, "src": pkg.Path}).Debug(iu.Package)
				...
				if iu.BinaryPath != "" {
					...
					Log.Log.WithFields(logrus.Fields{"from": iu.BinaryPath, "to": bpath}).Trace("copied binary")
					if err != nil {
						Log.Log.WithFields(logrus.Fields{"from": iu.BinaryPath, "to": bpath}).Error("error copying binary")
					}
				}
			}
			wg.Done()
	...
	go func(c chan string) {
		...
		for p := range c {
			...
			pkg, _ := dl.Get(p)
			Log.Log.WithField("package", p).Trace("pushing installation to queue")
	...
	Log.Log.WithField("packages", strings.Join(plan.StartingPackages, ", ")).Info("starting initial install")
	...
	Log.Log.WithField("duration", time.Since(startTime)).Info("total install time")
	for pkg := range plan.DepDb {
		_, exists := installedPkgs[pkg]
		if !exists {
			Log.Log.Errorf("did not install %s", pkg)
		}
	}
	if anyFailed {
		Log.Log.Errorf("installation failed for packages: %s", strings.Join(failedPkgs, ", "))
